...
{p 4 8}{cmd:eci:} Economic Complexity Index (Variable at the location level). Its is diversity weighted by the ubiquity of each industry using the method of reflections. {p_end}
...
{p 4 8}{cmd:density:} Measures the presence of the location in the neighborhood of the industry. The more dense, the closest is a given industry to the economic structure of the location. (Variable at the location-industry level). Density is used to define a (somewhat more intuitive) distance measure as 1/density or 1-density. {p_end}

{p 4 8}{cmd:coi:} Complexity Outlook Index (Variable at the location level, formerly call Opportunity Value). Its the value of developing the remaining industries weighted by the distance (inverse of density) from the current economic structure. {p_end}

{p 4 8}{cmd:cog:} Complexity Outlook Gain (Variable at the Industry level, formerly call Opportunity Gain). Its the increase in Complexity Outlook Index for the country that will result from developing a given industry. {p_end}

{p 4 8}{cmd:diversity:} Diversity as the number of industries produced in a given location (Variable at the location level).

{p 4 8}{cmd:ubiquity:} Number of locations where the industry is present (Variable at the industry level). {p_end}


{p 4 8}{cmd:rca:} Balassa’s Revealed Comparative Advantage index. Is the share of the industry in the location’s production, over the importance of the industry in the whole sample (Variable at the location-industry level. only provided if the RCA option is used). {p_end}
...
{p 4 8}{cmd:M:} Presence-absence matrix. Is the result of thresholding either the RCA or RPOP variables. This is, if industry intensity is over the given threshold the variable will show a 1, and zero otherwise (Variable at the location-industry level). {p_end}
...
{p 4 8}{cmd:rca(}#{cmd:)}, specifies the RCA threshold above which the calculations assume that a industry is present in location i. If not specified, RCA>=1 is the default. If both {cmd:rca(}#{cmd:)} and {cmd:rpop(}#{cmd:)} are not specified, the program will asume the using RCA and a threshold of 1 by default {p_end}

{p 4 8}{cmd:rpop(}#{cmd:)}, specifies the Rpop threshold above which the calculations assume that a industry is present in location i. If not specified, Rpop>=1 is the default. It requires specifying the population variable {p_end}

{p 4 8}{cmdab:pop(}{it:variable4}{cmd:)}, specifies name of the variable that will be used for the Rpop calculations. {p_end}

{p 8 8} Specifying both {cmd:rca(}#{cmd:)} and {cmd:rpop(}#{cmd:)},  will result in the combination the two criteria. ie the Mcp matrices used for calculations will consider that a industry is present in a location if either measure is above the corresponding threshold. {p_end}

{p 4 8}{cmdab:cont}, This option tells the program to calculate the proximity matrix as the result of the Pearson Correlation of every pair of industrys.  By default the program uses the probability of industry co-ocurrence to building the proximity matrices. {p_end}

{p 4 8}{cmdab:asym}, This option tells the program to calculate an asymmetric matrix of proximites. (i.e. the probability of observing industry A and B is different from observing B and A. Obviously, it does not work with continuous (Pearson correlation) proximities, which by definition are symmetric.  {p_end}
